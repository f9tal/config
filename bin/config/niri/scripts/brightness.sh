...
# Find the first available brightness device
device=$(brightnessctl -l -m | head -n 1 | cut -d ',' -f 1)

if [ -z "$device" ]; then
    notify "Error" "No brightness device found."
    exit 1
fi
...
case "$1" in
    ...
    down)
        ...
        ;;
    *)
        notify "Script Error" "Usage: brightness.sh {up|down}"
        exit 1
        ;;
esac